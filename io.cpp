...
void GetOptions(run_params &p, const int argc, const char **argv)
{
    ...
    {
        p_switch = argv[x];
        if (p_switch.compare("--replicas") == 0)
        {
            x++;
        ...
        else if (p_switch.compare("--no_limit") == 0)
        { // Remove the limit on the population size increase
            x++;
...
void ImportDetections(const run_params &p, int &n_detections, std::vector<detect> &detections)
{
    ...
    for (unsigned long i = 0; i < 1000000; i++)
    {
        ...
        n_detections = n_detections + d.cases;
    }
}
...
void OutputOutbreakDeathStatistics(const run_params &p, const std::vector<int> &timepoints, const std::vector<std::vector<output>> &results)
{
    ...
    { // For each of the time points
        ...
        for (unsigned long r0val = 0; r0val < p.R0_vals.size(); r0val++)
        {
            n_dead += results[i][r0val].dead;
            n_accepted += results[i][r0val].accepted;
        }
        double prob = static_cast<double>(n_dead) / static_cast<double>(n_accepted);
...
void OutputOutbreakPopulationStatistics(const run_params &p, const std::vector<int> &timepoints, const std::vector<std::vector<output>> &results)
{
    ...
    for (unsigned long i = 0; i < timepoints.size(); i++)
    {  // For each of the time points, counts of the population sizes for each R0
        ...
        for (unsigned long r0val = 0; r0val < p.R0_vals.size(); r0val++)
        {
            ...
            {
                // Extend size vector if too small
        ...
        // Normalize this size distribution
        double tot = 0;
